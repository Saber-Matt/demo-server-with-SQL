...
// cats
app.post('/api/snek', async (req, res) => {
  try {
    const snek = req.body;
    ...
    `, [
      snek.name, snek.type, snek.url, snek.species, snek.accessory, snek.isDeadlyWithTheVenom, snek.userId]);
...
app.put('/api/snek/:id', async (req, res) => {
  try {
    const snek = req.body;
...
app.delete('/api/snek/:id', async (req, res) => {
  try {
    const data = await client.query(`
      DELETE FROM  snek 
      WHERE id = $1
...
app.get('/api/snek', async (req, res) => {
